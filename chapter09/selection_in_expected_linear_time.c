...
{
	int candidate = rand() % (high+1-low) + low;
...
int selectn(int a[], int low, int high, int select_idx)
{
	if (low >= high) return low;
	int mid = random_partition(a, low, high);
	if (mid == select_idx) return mid;
	if (mid > select_idx) return selectn(a, low, mid-1, select_idx);
	return selectn(a, mid+1, high, select_idx);
}
